...
>> The chance his brother was indentical is 29.4%. This is found by dividing the unconditional chance he had a indentical brother by the total chance he had a twin. (1/300)/(1/125+1/300).
...
>> A Frequentist takes the position that probabilities are only appropriate for events that can be repeated, at least in principle. They take probability to be the frequency at which an event is likely to occur over successive trials. Bayesians are inclined to place a probability distribution over anything about which there is uncertainty. These distributions gives rise to "Bayesian priors". Bayesian priors, at their core, cannot be based on frequency data and are thus rejected by Frequentists as unscientific.  

>> The difference between the two approaches can be seen in the case of testing a hypothesis that a parameter is positive. Under a Bayesian approach, one can say directly that the particular a parameter is positive with a probability greater than 0.05. Such a calculation requires a Bayesian prior over the parameter in question which is updated with use of the data. A Frequentist does not use a Bayesian prior and therefore cannot describe the probability the parameter is in some range of values. Thus the frequentist takes a more indirect approach: one rejects a hypothesis that parameter is negative by noting that the probability of observing the data given a negative parameter is less than 0.05. From a practical perspective, the two approaches are used in the same way, one concludes the parameter is statistically significantly greater than zero. However, note that the Frequentist did not answer the same question. Whatever approach one chooses there is a tough pill to swallow, the unscientific prior or the indirect hypothesis testing.
