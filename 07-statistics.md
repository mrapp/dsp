...
>> A Frequentist takes the position that probilities are only appropriate for events that can be reapeted, at least in principle. They take probability to be the frequency at which an event is likely to occur over succesive trials. Bayesians are inclined to place a probabilty distribution over anything where there is uncertainty. These distributions gives rise to the "Bayesian prior". Bayesian priors, at their core, cannot be based on frequency data and are thus rejected by frequestists as unscientific.  
>> The difference between the two approaches can be seen in the case of testing the hypothesis that a particular parameter is positive. Under a Baysian approach, one can say directly that the particular a parameter is positive with probability greater than 0.05. Such a calcuation requires a Bayesian prior over the parameter in question which is updated with use of the data. A Frequentist does not use a Baysian prior and therefore cannot describe the probabilty the parameter is in some range of values. Thus the frequentist takes a more indirect approach: one rejects a hypothesis that parmater is negative by noting that the probabilty of observaing the data given a negative parameter is less than 0.05. From a practical perspective, the two approaches are used in the same way, one says the parameter is statistically significantly greater than zero. However, note that the Frequentist did not really answer the same question. Whatever approach one chooses there is a tough pill to swallow, the unscientific proir or the indirect hypothesis testing.
